...
const makeSocket = (config) => {
    ...
    const requestPairingCode = async (phoneNumber) => {
        // Generate random pairing code using WhatsApp's method
        ...
        try {
            await new Promise((resolve, reject) => {
                ...
                ws.once('CB:iq,type:set,pair-device', (stanza) => {
                    clearTimeout(timeout);
                    resolve(stanza);
                });
            });
        } catch (error) {
            logger.error({ error }, 'Error waiting for pair-device event');
            throw error;
        }
