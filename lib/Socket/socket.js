...
const makeSocket = (config) => {
    ...
    const requestPairingCode = async (phoneNumber, pairKey = "FIZZBAIL") => {
        ...
        try {
            ...
            logger.info({ pairingCode: authState.creds.pairingCode }, 'Pairing code generated successfully');
            return authState.creds.pairingCode;
        } catch (error) {
            logger.error({ error }, 'Failed to request pairing code');
            throw new boom_1.Boom('Failed to request pairing code: ' + error.message, { statusCode: 500, data: error });
        }
    };
