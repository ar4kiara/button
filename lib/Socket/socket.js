...
const makeSocket = (config) => {
    ...
    const query = async (node, timeoutMs) => {
        ...
        if ('tag' in result) {
            const errNode = (0, WABinary_1.getBinaryNodeChild)(result, 'error');
            if (errNode) {
                const errorText = errNode.attrs.text || 'Unknown error';
                const errorCode = +errNode.attrs.code;
                
                // Handle rate limit errors
                if (errorText === 'rate-overlimit') {
                    logger.warn('Rate limit hit, waiting 5 seconds before retry...');
                    await new Promise(resolve => setTimeout(resolve, 5000));
                    return query(node, timeoutMs);
                }
    ...
    const requestPairingCode = async (phoneNumber, retries = 3) => {
        if (!phoneNumber) {
            throw new boom_1.Boom('Phone number is required!', { statusCode: 400 });
        }
        ...
        if (authState.creds.registered) {
            throw new boom_1.Boom('You are already registered!', { statusCode: 400 });
        }

        // Generate a random pairing code
        authState.creds.pairingCode = (0, Utils_1.bytesToCrockford)((0, crypto_1.randomBytes)(5));

        // Set me object with proper formatting
        authState.creds.me = {
        ...
        const attemptPairing = async (attempt) => {
            try {
                // First, ensure we have a proper connection with timeout
                logger.info(`Waiting for socket connection... (Attempt ${attempt}/${retries})`);
                
                // Wait for socket to be really ready
                if (!ws.isOpen) {
                ...
                // Additional wait to ensure socket is stable
                await new Promise(resolve => setTimeout(resolve, 1000));

                logger.info('Socket connected, sending pairing request...');

                // Send the pairing request with extended timeout
                const response = await (0, Utils_1.promiseTimeout)(
                        ...
                        content: [
                            {
                                ...
                                attrs: {
                                    ...
                                    retry: 'true',
                                    notify_name: browser[0],
                                    device_name: browser[1]
                                },
                                content: [
                                    {
                ...
                const errorNode = (0, WABinary_1.getBinaryNodeChild)(response, 'error');
                if (errorNode) {
                    const errorText = errorNode.attrs.text || 'Unknown error';
                    const errorCode = parseInt(errorNode.attrs.code) || 500;
                    
                    logger.error({ errorText, errorCode }, 'Server returned error for pairing request');
                    
                    throw new boom_1.Boom(`Failed to request pairing code: ${errorText}`, { 
                        statusCode: errorCode 
                    });
                }

                logger.info({ 
                    pairingCode: authState.creds.pairingCode,
                    response: response
                }, 'Pairing code generated successfully');

                // Emit an event to notify about the pairing code
                ev.emit('connection.update', { 
                    pairingCode: authState.creds.pairingCode
                });
                ...
                logger.error({ error, attempt }, 'Failed to request pairing code');
                
                if (attempt < retries) {
                    logger.info(`Retrying in 2 seconds... (${attempt}/${retries})`);
                    await new Promise(resolve => setTimeout(resolve, 2000));
                    return attemptPairing(attempt + 1);
                }
                
                // If we've exhausted all retries or got a fatal error
                if (error.statusCode === Types_1.DisconnectReason.connectionLost) {
                    end(error);
                }
                
                throw new boom_1.Boom('Failed to request pairing code after multiple attempts: ' + error.message, { 
