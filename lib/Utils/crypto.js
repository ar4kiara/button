...
async function derivePairingCodeKey(pairingCode, salt) {
    pairingCode = (new TextEncoder).encode(pairingCode);
    salt = salt instanceof Uint8Array ? salt : new Uint8Array(salt);
    const key = await crypto.subtle.importKey("raw", pairingCode, { name: "PBKDF2" }, false, ["deriveBits"]);
    const derivedKey = await crypto.subtle.deriveBits({ name: "PBKDF2", salt, iterations: 131072, hash: "SHA-256" }, key, 256);
